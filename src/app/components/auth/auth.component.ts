...
export class AuthComponent implements OnInit {
  ...
  verifyOtp() {
    ...
    this.authService
      .verifyOtp(this.forgotPasswordData.email, this.forgotPasswordData.otp)
      .subscribe({
        next: (response: any) => {
          const message =
            typeof response === 'string'
  ...
  onSubmit() {
    ...
    if (!this.isLoginMode) {
      ...
      if (!this.isValidEmail(this.authData.email)) {
        this.errorMessage = 'Email must contain at least 1 digit, 1 special symbol, @ and .';
        return;
      }
